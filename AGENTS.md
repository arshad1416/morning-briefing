...
| `pi-scripts/` | `python3 -m unittest test_fetch_universe_constituents test_universe_membership test_volume_nan` | runs locally without the Pi venv (stubs yfinance/pandas); CI runs exactly this set. Other `test_*.py` here need the Pi venv — don't use `unittest discover` |

...
const Account = {
  async render(app) {
    ...
    if (me) {
      ...
      const isSub = ent.entitled && (ent.tier === 'basic' || ent.tier === 'pro');
      const canceled = ent.status === 'canceled';
      const planLine = isSub
        ? `<div class="auth-sub">Plan: <strong>${ent.tier === 'pro' ? 'Pro' : 'Basic'}</strong>${canceled ? ' — cancels at period end' : ''}</div>`
        : (ent.tier === 'trial' && ent.entitled ? '<div class="auth-sub">Free trial active</div>' : '');
      app.innerHTML = `
        <div class="auth-wrap">
          <div class="auth-card">
            ...
            <div class="auth-actions">
              ...
              ${isSub && !canceled ? '<button class="btn btn-secondary" id="cancel-sub">Cancel subscription</button>' : ''}

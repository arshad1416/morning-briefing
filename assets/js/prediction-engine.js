...
const PredictionEngine = {
  ...
  async render(app) {
    ...
    // QWEN-ADD: Enhanced accuracy with expectancy, drawdown, slippage
    queue.push(() => PredictionEngine._renderEnhancedAccuracy(app));
  ...
  /**
   * QWEN-ADD: Enhanced accuracy display with expectancy, max drawdown, slippage.
   * Reads from /data/accuracy.json (generated by generate_prediction_accuracy.py).
   */
  async _renderEnhancedAccuracy(app) {
    try {
      const data = await Utils.fetchJSON('/data/accuracy.json');
      if (!data || !data.expectancy) return;
      ...
      // Color helpers
      const expColor = exp.expectancy_pct > 0 ? 'var(--green)' : 'var(--red)';
      const ddColor = dd.max_drawdown_pct < 10 ? 'var(--green)' : dd.max_drawdown_pct < 20 ? 'var(--yellow)' : 'var(--red)';

      let html = '<h2 class="section-title">Live Performance Metrics<span style="flex:1"></span><span style="font-size:0.65rem;color:var(--text-muted);font-weight:400;text-transform:none">Qwen Enhanced</span></h2>';

      // Key metrics cards
      html += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:8px;margin-bottom:16px">';

      // Expectancy
      html += '<div style="text-align:center;padding:12px;background:var(--bg-inset);border-radius:var(--card-radius);border:1px solid var(--border-dim)">';
      html += '<div style="font-size:0.6rem;color:var(--text-muted);text-transform:uppercase">Expectancy</div>';
      html += `<div style="font-size:1.2rem;font-weight:700;color:${expColor}">${exp.expectancy_pct > 0 ? '+' : ''}${exp.expectancy_pct}%</div>`;
      html += `<div style="font-size:0.6rem;color:var(--text-muted)">per trade</div></div>`;

      // Profit Factor
      html += '<div style="text-align:center;padding:12px;background:var(--bg-inset);border-radius:var(--card-radius);border:1px solid var(--border-dim)">';
      html += '<div style="font-size:0.6rem;color:var(--text-muted);text-transform:uppercase">Profit Factor</div>';
      html += `<div style="font-size:1.2rem;font-weight:700;color:${exp.profit_factor >= 1.5 ? 'var(--green)' : 'var(--yellow)'}">${typeof exp.profit_factor === 'number' ? exp.profit_factor.toFixed(2) : '∞'}</div>`;
      html += '<div style="font-size:0.6rem;color:var(--text-muted)">wins/losses</div></div>';

      // Max Drawdown
      html += '<div style="text-align:center;padding:12px;background:var(--bg-inset);border-radius:var(--card-radius);border:1px solid var(--border-dim)">';
      html += '<div style="font-size:0.6rem;color:var(--text-muted);text-transform:uppercase">Max Drawdown</div>';
      html += `<div style="font-size:1.2rem;font-weight:700;color:${ddColor}">-${dd.max_drawdown_pct || 0}%</div>`;
      html += `<div style="font-size:0.6rem;color:var(--text-muted)">${dd.drawdown_duration_trades || 0} trades</div></div>`;

      // Kelly
      html += '<div style="text-align:center;padding:12px;background:var(--bg-inset);border-radius:var(--card-radius);border:1px solid var(--border-dim)">';
      html += '<div style="font-size:0.6rem;color:var(--text-muted);text-transform:uppercase">Kelly %</div>';
      html += `<div style="font-size:1.2rem;font-weight:700">${exp.kelly_fraction || 0}%</div>`;
      html += '<div style="font-size:0.6rem;color:var(--text-muted)">optimal sizing</div></div>';

      // Slippage
      if (slip.n_measured > 0) {
        html += '<div style="text-align:center;padding:12px;background:var(--bg-inset);border-radius:var(--card-radius);border:1px solid var(--border-dim)">';
        html += '<div style="font-size:0.6rem;color:var(--text-muted);text-transform:uppercase">Avg Slippage</div>';
        html += `<div style="font-size:1.2rem;font-weight:700;color:var(--yellow)">${slip.avg_slippage_pct}%</div>`;
        html += `<div style="font-size:0.6rem;color:var(--text-muted)">${slip.n_measured} measured</div></div>`;
      }
      ...
      if (perStrat.length > 0) {
        html += '<div class="card" style="padding:0;overflow:hidden;margin-bottom:16px"><table style="width:100%;border-collapse:collapse;font-size:0.75rem"><thead><tr>';
        html += '<th style="text-align:left;padding:6px 8px;border-bottom:1px solid var(--border-subtle);color:var(--text-muted)">Strategy</th>';
        html += '<th style="text-align:center;padding:6px 8px;border-bottom:1px solid var(--border-subtle);color:var(--text-muted)">Trades</th>';
        html += '<th style="text-align:center;padding:6px 8px;border-bottom:1px solid var(--border-subtle);color:var(--text-muted)">WR</th>';
        html += '<th style="text-align:center;padding:6px 8px;border-bottom:1px solid var(--border-subtle);color:var(--text-muted)">Expectancy</th>';
        html += '<th style="text-align:center;padding:6px 8px;border-bottom:1px solid var(--border-subtle);color:var(--text-muted)">PF</th>';
        html += '<th style="text-align:center;padding:6px 8px;border-bottom:1px solid var(--border-subtle);color:var(--text-muted)">Max DD</th>';
        html += '<th style="text-align:center;padding:6px 8px;border-bottom:1px solid var(--border-subtle);color:var(--text-muted)">Status</th>';
        html += '</tr></thead><tbody>';

        perStrat.forEach(function(s) {
          const sExpColor = s.expectancy_pct > 0 ? 'var(--green)' : 'var(--red)';
          const sDdColor = s.max_drawdown_pct < 10 ? 'var(--green)' : s.max_drawdown_pct < 20 ? 'var(--yellow)' : 'var(--red)';
          const statusBadge = s.status === 'profitable'
            ? '<span class="badge badge-green" style="font-size:0.6rem">Profitable</span>'
            : '<span class="badge badge-red" style="font-size:0.6rem">Losing</span>';

          html += '<tr>';
          html += '<td style="padding:5px 8px;border-bottom:1px solid var(--border-subtle)"><strong>' + Utils.esc(s.strategy) + '</strong></td>';
          html += '<td style="text-align:center;padding:5px 8px;border-bottom:1px solid var(--border-subtle)">' + s.n_trades + '</td>';
          html += '<td style="text-align:center;padding:5px 8px;border-bottom:1px solid var(--border-subtle)">' + s.win_rate + '%</td>';
          html += '<td style="text-align:center;padding:5px 8px;border-bottom:1px solid var(--border-subtle);color:' + sExpColor + ';font-weight:600">' + (s.expectancy_pct > 0 ? '+' : '') + s.expectancy_pct + '%</td>';
          html += '<td style="text-align:center;padding:5px 8px;border-bottom:1px solid var(--border-subtle)">' + (typeof s.profit_factor === 'number' ? s.profit_factor.toFixed(2) : '∞') + '</td>';
          html += '<td style="text-align:center;padding:5px 8px;border-bottom:1px solid var(--border-subtle);color:' + sDdColor + '">-' + s.max_drawdown_pct + '%</td>';
          html += '<td style="text-align:center;padding:5px 8px;border-bottom:1px solid var(--border-subtle)">' + statusBadge + '</td>';
          html += '</tr>';
        });
      ...
      // Silently skip if accuracy.json doesn't exist
    }
  }
};
